...
def read(top, main_root, use_bd=True):
    ...
    bd, jsonl or none. bd: the CLI path the model may run, set only when `bd list` worked here and there is no
    note. note: why bd is off although .beads exists."""
    beads_dir = find_beads(top, main_root)
    if beads_dir is None:
        return dict.fromkeys(("state", "source", "bd", "note"))
    ...
    if env_dir and norm(env_dir) != norm(beads_dir):  # the model's own bd calls would go to another repo's DB
        note = "BEADS_DIR points outside this repo; unset it to use beads here"
    elif not use_bd:
        note = "bd was skipped (--no-bd)"
    elif not bd:
        note = "the bd CLI was not found"
    elif source != "bd":
        note = "`bd list` failed here"
    else:
        note = None
    return {"state": beads_state(issues), "source": source, "bd": None if note else bd, "note": note}

...
Autopilot: an open plan phase goes to a detached `yah run --plan` (section 3), which goes on after this session closes. The task `here` opts out: where a row says **start the run**, run `yah:start` hands-on instead, with the NEXT line or the next phase's label and title.
...
| A task was given above, other than `here` | It is the task. If it needs several PRs or sessions, plan it in plan mode; once the user approves, run `yah:phases`, then start its first phase. Otherwise run `yah:start` with the task. |

...
allowed-tools: Bash(python3 *scripts/where.py*), Bash(python *scripts/where.py*), Bash(py -3 *scripts/where.py*), Bash(gh pr view *), Bash(gh pr checks *)
---
...
| The phase's PR has failing checks | Read `gh pr checks <N>` and the failed log, fix the cause, run the tests, push the phase branch. |
...
| You are not on the phase's branch | Switch to it if the tree is clean; if not, say so and ask. |
...
- A hard question, or a bug that survived two attempts: run `yah:deep` with a question that stands alone (goal, files, what was tried, what happened). This holds on every plan tier. Without Fable access, Claude Code runs it on the session's model.

...
| NEXT waits on the user (a PR to merge, "after PR #N merges") | `gh pr view <N> --json state`. Merged: go on with NEXT (usually update the base branch, then branch). Not merged: say in one line what waits on the user, with the command that does it (`gh pr merge <N> --merge`, as the PR line shows), then ask: wait, or start the next phase stacked on this branch. |

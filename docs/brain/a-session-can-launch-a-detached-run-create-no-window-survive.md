...
- Settled once --detach existed: auto mode let this session's Bash tool run `run.py P2 --detach --cwd <scratch repo>`. The call returned at once, the run was still alive (holding its lock) in the next tool call, and it started a real headless claude -p before it stopped on its own.
- POSIX start_new_session and flock pass under WSL Ubuntu 24.04 (tests/test_run.py detach tests).
